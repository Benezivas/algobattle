...
class Ui(Observer):
    ...
    def output_formatted_ascii(self, match_data: dict):
        ...
        if match_data['type'] != 'iterated':
            ...
            raise Exception('The UI can only handle the battle_type "iterated"! Type given: "{}"'.format(match_data['type']))
        ...
        for match in match_data.keys():
            if isinstance(match, tuple):
                ...
                avg = sum(match_data[match][i]['solved'] for i in range(match_data['iterations'])) // match_data['iterations']

                print('║{:>9s}║{:>9s}'.format(match[0], match[1])
                      + ''.join(['║{:>6d}'.format(match_data[match][i]['solved']) for i in range(match_data['iterations'])])
                      + '║{:>6d}║{:>6d}║{:>6s}║'.format(match_data[match][curr_iter]['cap'], avg, "") + '\r')
        print('╚═════════╩═════════╩'
              + ''.join(['══════╩' for i in range(match_data['iterations'])])
              + '══════╩══════╩══════╝' + '\n\r')
