...
class FightHandler:
    ...
    def fight(self, instance_size: int) -> float:
        ...
        logger.info(
            f"Solver of group {self.solving_team} yields a valid solution with an approx. ratio of {approximation_ratio}."
        )
    ...
    def _run_generator(self, instance_size: int) -> Tuple[Any, Any]:
        ...
        try:
            ...
            encoded_output = self.generating_team.generator.run(
                str(instance_size), self.timeout_generator, scaled_memory, self.cpus
            )
        except DockerError:
    ...
    def _run_solver(self, instance_size: int, instance: Any) -> Any:
        ...
        try:
            ...
            encoded_output = self.solving_team.solver.run(
        ...
        if not self.problem.verifier.verify_semantics_of_solution(solver_solution, instance_size, True):
            logger.warning(
                f"Solver of group {self.solving_team} created a malformed solution at instance size {instance_size}!"
            )
            raise RuntimeError
        elif not self.problem.verifier.verify_solution_against_instance(instance, solver_solution, instance_size, False):
            logger.warning(
                f"Solver of group {self.solving_team} yields an incorrect solution at instance size {instance_size}!"
            )
            raise RuntimeError
