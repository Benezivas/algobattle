...
logger = logging.getLogger('algobattle.problems.domset.parser')
...
class DomsetParser(Parser):
    ...
    def parse_instance(self, raw_instance, instance_size):
        ...
        for line in raw_instance:
            if len(line) != 3:
                logger.warning('An edge line is of unexpected length!')
                ...
                logger.warning('An edge is not in the allowed range!')
                removable_lines.append(line)
