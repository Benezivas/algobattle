...
class Tests(Problem[list[list[str]], list[str]]):
    ...
    def parse_instance(instance: list[str], size: int) -> list[list[str]]:
        parsed = [line.split(" ") for line in instance]
        if not instance:
            raise ValueError
    ...
    def parse_solution(solution: list[str], size: int) -> list[list[str]]:
        parsed = [line.split(" ") for line in solution]
        if not parsed:
            raise ValueError
    ...
    @staticmethod
    def verify_solution(instance: list[str], size: int, solution: str) -> bool:
    ...
    @staticmethod
    def solution_weight(instance: list[str], size: int, solutoion: str) -> float:
